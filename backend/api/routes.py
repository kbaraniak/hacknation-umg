...
async def compare_branches(
	codes: str = Query(..., description="Lista kodów PKD lub sekcji oddzielonych przecinkami"),
	version: Optional[str] = Query("2025", description="Wersja PKD"),
	years: Optional[str] = Query(None, description="Zakres lat, np. 2020-2024")
):
	"""
	Porównaj wiele branż jednocześnie.
	Zwraca zagregowane dane finansowe dla podanych kodów.
	"""
	try:
		pkd_version = PKDVersion.VERSION_2025 if version == "2025" else PKDVersion.VERSION_2007
		hierarchy = service.loader.get_hierarchy(pkd_version)
		...
		for code_str in code_list:
			# Determine hierarchy params
			section, division, group, subclass = None, None, None, None
			
			# Handle frontend format: Section.Division.Suffix (e.g. E.38.38.12 or A.03.03.11)
			parts = code_str.split('.')
			parsed_code = code_str
			
			if len(parts) >= 3 and len(parts[0]) == 1 and parts[0].isalpha() and len(parts[1]) == 2 and parts[1].isdigit():
				# Format: Section.Division.Suffix
				# Suffix is everything after the second dot
				parsed_code = ".".join(parts[2:])
			elif len(parts) == 2 and len(parts[0]) == 1 and parts[0].isalpha() and len(parts[1]) == 2 and parts[1].isdigit():
				# Format: Section.Division (e.g. G.47)
				parsed_code = parts[1]
			
			if len(parsed_code) == 1 and parsed_code.isalpha():
				section = parsed_code.upper()
			elif len(parsed_code) == 2 and parsed_code.isdigit():
				division = parsed_code
				# Find section
				candidates = hierarchy.get_by_division(division)
				if candidates:
					section = candidates[0].section
			else:
				# Try to find exact code
				pkd_code = hierarchy.get_by_symbol(parsed_code)
				if pkd_code:
					section = pkd_code.section
					division = pkd_code.division
					group = pkd_code.group
					subclass = pkd_code.subclass
				else:
					# Try to find as group (e.g. 46.1)
					candidates = hierarchy.get_by_group(parsed_code)
					if candidates:
						c = candidates[0]
						section = c.section
						division = c.division
